...
public interface RewardSalesRepository extends JpaRepository<RewardSalesDAO, Long> {
    ...
    @Query(value = "SELECT COALESCE(rs1, 0) - COALESCE(rs2, 0) FROM "
            + "(SELECT SUM(rp1.amount) AS rs1 FROM RewardSalesDAO AS rp1 WHERE rp1.type = 'deposit' AND rp1.userDAOId = :uid) AS subquery1,"
            + "(SELECT SUM(rp2.amount) AS rs2 FROM RewardSalesDAO AS rp2 WHERE rp2.type = 'undeposit' AND rp2.userDAOId = :uid) AS subquery2")
